...
impl<I2C> ICM42688<I2C, Uninitialized> {
    /// Create a new instance of `DW3000`
    ///
    /// Requires the I2C peripheral and the chip select pin that are connected
    /// to the DW3000.
    pub fn new(i2c: I2C) -> Self {
        ICM42688 {
            ll: crate::ll::ICM42688::new(i2c),
            _state: Uninitialized,
        }
    }

    #[cfg(feature = "async")]
    pub async fn initialize(mut self, mut delay: impl DelayNs) -> Result<ICM42688<I2C, Ready>, ()>
    where
        I2C: I2c,
    {
        use crate::Ready;
        ...
        bank0
            .intf_config0()
            .async_modify(|_, w| w.fifo_count_endian(1).sensor_data_endian(1).ui_sifs_cfg(10))
            .await
            .unwrap();
        bank0
            .intf_config1()
    ...
    #[cfg(not(feature = "async"))]
    pub fn initialize(mut self, mut delay: impl DelayNs) -> Result<ICM42688<I2C, Ready>, ()>
    where
        I2C: Write + WriteRead,
    {
        use crate::Ready;
        ...
        bank0
            .intf_config0()
            .modify(|_, w| w.fifo_count_endian(1).sensor_data_endian(1).ui_sifs_cfg(10))
            .unwrap();
        bank0
            .intf_config1()
